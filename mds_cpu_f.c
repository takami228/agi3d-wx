...
int main (int argc, char * argv[]){
    ...
    float * D2;
    D2 = malloc((long)sizeof(float)*(N)*(N));
    ...
    for(i = 0; i < N; i++){
        D2[i+i*N] = 0.0;
        for(j = i+1; j < N; j++){
            D2[i+j*N] = D2[j+i*N] = (float)(D[i+j*N]*D[i+j*N]);
            if(D[i+j*N] == 1) edge++;
        }
    }
    ...
    s = getETime();
    float * b; float * H;
    b = malloc((long)sizeof(float)*(N)*(N));
    H = malloc((long)sizeof(float)*(N)*(N));
    ...
    float * A;
    float * w;
    A = malloc((long)sizeof(float) * (N) * (N));
    w = malloc((long)sizeof(float) * (N));
    ...
    info = LAPACKE_ssyev(LAPACK_COL_MAJOR, 'V', 'U', n, A, n, w);
    ...
    float * lambdas;
    lambdas = malloc((long)sizeof(float)*dim);
    ...
    float * U;
    U = malloc((long)sizeof(float)*N*N);
    ...
    float * P; float * L; 
    P = malloc((long)sizeof(float)*N*dim);
    L = malloc((long)sizeof(float)*N*dim);

    for(i = 0; i < N; i++){
        for(j = 0; j < dim; j++){
            if(i == j) L[i+j*N] = (float)sqrt(lambdas[j]);
            else L[i+j*N] = 0.0;
            P[i+j*N] = 0.0;
        }
    }
