...
static float phi = 0, theta = 0, psi;
...
void DrawPanel::RenderScene(wxPaintEvent& evt){
    ...
    //Set View
    {
        ...
        if(LayoutMode == 2){
            cml::matrix_rotation_world_z(zRot,psi);
            up.set(0.0f,1.0f,0.0f);
            vector4 _up(up[0],up[1],up[2],1);
            _up = zRot*_up;
            up.set(_up[0],_up[1],_up[2]);
...
/*
    ...
    glPushMatrix();
    glScalef((GLfloat)2, (GLfloat)2, (GLfloat)2);
    glDrawElements(GL_TRIANGLES, points, GL_UNSIGNED_INT, BUFFER_OFFSET(0));
    glPopMatrix();

    glLineWidth((GLfloat)6.0f);
...
float DrawPanel::UpdateNodeThreshold_b(float t, int attrID){
    ...
    if(id != -1){
        ...
        for(int i = 0; i < neighbor[id].size(); i++){
            isdrawingNodes[neighbor[id][i]] = true;
        }
    }*/
    Refresh();
...
void DrawPanel::UpdateEdgeThreshold_b(float t, int attrID){
    ...
    if(id != -1){
        for(int i = 0; i < edgelist[id].size(); i++){
            isdrawingNodes[edgelist[id][i]] = true;
        }
    }*/
    Refresh();
}
...
void DrawPanel::CalcStressValue(){
    if(LayoutMode == 3){
        calcStress3D();
    }
    else if(LayoutMode == 2){
...
int DrawPanel::ProcessSelection(int xPos, int yPos){
    ...
    float depth = returnDepth(xPos, yPos);
    double _x, _y, _z;
...
void DrawPanel::mouseReleased(wxMouseEvent& event) {
    if(LOAD_FLAG){
        isDrag = false;
        Refresh();
    }
...
void DrawPanel::onKeyDown(wxKeyEvent& event){
    if(LOAD_FLAG){
        ...
        switch(code){
            case 8: target.set(0,0,0);
            case 82: ResetLayout(); break;
            case 80: SavePixelData(); break;
            case 76: PrintLayout(); break;
            case 67: CalcStressValue(); break;
            case 65: Run(); break;
            default: break;
        }
    }
...
void DrawPanel::onKeyUp(wxKeyEvent& event){
    //cout << event.GetKeyCode() << " Key Up" << endl;
    //cout << WXK_LEFT << " " << WXK_RIGHT << endl;
    //cout << c << endl;
}